...
class ApiService {
  ...
  /**
   * Save any video as favorite (creates trend on-the-fly if needed)
   * POST /api/favorites/save-video
   */
  async saveVideoAsFavorite(data: {
    platform_id: string;
    url?: string;
    description?: string;
    cover_url?: string;
    ...
    uts_score?: number;
    search_query?: string;
  }): Promise<{ trend_id: number; favorite_id: number; already_saved: boolean }> {
  ...
  /**
   * Analyze a video using Gemini Vision (downloads + analyzes actual video)
  ...
  async getWorkflow(id: number): Promise<{
    id: number;
    name: string;
    description?: string;
  ...
  async getWorkflowHistory(limit = 50, offset = 0): Promise<any[]> {
