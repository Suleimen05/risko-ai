...
export function WorkflowProvider({ children }: { children: ReactNode }) {
  ...
  // Create new workflow
  const createWorkflow = useCallback(async (name?: string): Promise<WorkflowData | null> => {
  ...
  const saveWorkflow = useCallback(async (data: {
    name?: string;
    graph_data?: { nodes: any[]; connections: any[] };
    node_configs?: Record<string, any>;
    canvas_state?: { zoom: number; panX: number; panY: number };
    ...
    try {
      ...
      setWorkflows(prev => prev.map(wf =>
        wf.id === currentWorkflow.id
          ? { ...wf, name: updated.name, updated_at: updated.updated_at, node_count: updated.graph_data?.nodes?.length || 0 }
          : wf
      ));
    } catch (error) {
  ...
  const scheduleAutoSave = useCallback((data: any) => {
    ...
    autoSaveTimerRef.current = setTimeout(async () => {
      if (pendingSaveRef.current && currentWorkflow) {
        await saveWorkflow(pendingSaveRef.current);
