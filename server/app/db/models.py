...
class ChatSession(Base):
    ...
    context_type = Column(String(50), nullable=True)  # 'video', 'trend', 'script'
...
class ChatMessage(Base):
    ...
    # Note: Using string matching instead of FK for backward compatibility
    ...
    # AI metadata
    model = Column(String(50), nullable=True)  # 'gemini', 'claude', 'gpt'
...
class Workflow(Base):
    ...
    # Per-node config overrides: {"nodeId": {"customPrompt": "...", "model": "gemini"}}
...
class WorkflowRun(Base):
    ...
    Similar to ChatSession/ChatMessage for chat history.
    """
    ...
    workflow_id = Column(
        ...
        nullable=True,  # Nullable so history persists if workflow is deleted
        index=True
    )

    # Run identification
    workflow_name = Column(String(255), nullable=False)  # Snapshot of name at run time
    run_number = Column(Integer, default=1, nullable=False)  # Sequential run number for this workflow
    ...
    # Input snapshot: the graph that was executed
    input_graph = Column(JSONB, default={}, nullable=False)  # {"nodes": [...], "connections": [...]}
    node_count = Column(Integer, default=0, nullable=False)

    # Results: per-node outputs
    results = Column(JSONB, default=[], nullable=False)  # [{node_id, node_type, content, success, error}, ...]
