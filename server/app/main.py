...
# Load .env from server/ first, then from parent directory (project root)
load_dotenv()  # server/.env
load_dotenv(Path(__file__).resolve().parent.parent.parent / '.env')  # 89/.env
...
logger.info(f"[KEY] APIFY TOKEN: {'FOUND' if token else 'MISSING (Check .env)'}")
...
from .api import trends, profiles, competitors, ai_scripts, proxy, favorites, chat_sessions, workflows
...
# Workflow routes (for AI workflow execution)
app.include_router(
    workflows.router,
...
async def startup_event():
    ...
    try:
        ...
        start_scheduler()
        logger.info("[STARTUP] Scheduler is running and waiting for tasks.")
    except Exception as e:
        logger.warning(f"[WARNING] Scheduler initialization failed: {e}")
        logger.warning("[WARNING] Continuing without scheduler - auto-rescan will be disabled")

    logger.info("[STARTUP] Rizko.ai Backend started successfully!")


@app.on_event("shutdown")
async def shutdown_event():
    """Cleanup on shutdown."""
    logger.info("[SHUTDOWN] Shutting down Rizko.ai Backend...")
...
if __name__ == "__main__":
    ...
    logger.info("[MAIN] Starting Rizko.ai Backend on http://0.0.0.0:8000")
