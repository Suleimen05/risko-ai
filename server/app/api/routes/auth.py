...
async def register(user_data: UserRegister, db: Session = Depends(get_db)):
    ...
    # Create new user with initial credits based on Free plan
    hashed_password = get_password_hash(user_data.password)
    from dateutil.relativedelta import relativedelta
    initial_credits = CreditManager.get_monthly_limit(SubscriptionTier.FREE)
    new_user = User(
        email=user_data.email,
        hashed_password=hashed_password,
        full_name=user_data.full_name,
        credits=initial_credits,
        credits_reset_at=datetime.utcnow() + relativedelta(months=1),
        is_active=True,
...
async def sync_oauth_user(data: OAuthSyncRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if user is None:
            # Create new user from OAuth data with initial credits
            from dateutil.relativedelta import relativedelta
            initial_credits = CreditManager.get_monthly_limit(SubscriptionTier.FREE)
            user = User(
                email=data.email,
...
async def dev_upgrade_subscription(
    ...
    # Update user subscription and reset credits to new plan's allocation
    from dateutil.relativedelta import relativedelta
    new_tier = tier_map[data.plan.lower()]
    current_user.subscription_tier = new_tier
    current_user.credits = CreditManager.get_monthly_limit(new_tier)
    current_user.credits_reset_at = datetime.utcnow() + relativedelta(months=1)
    db.commit()
