...
from ..api.dependencies import CreditManager
...
async def analyze_video(
    ...
    credit_manager.check_and_reset_monthly(current_user)
...
def process_video_node(node: WorkflowNode) -> str:
    ...
    Falls back to text-only analysis if video download fails.
    """
    if not node.videoData:
        return "⚠️ No video attached. Drag a saved video onto this node."
    ...
    # If we have a URL, try real video analysis with Gemini
    if video.url:
        try:
            from ..services.video_analyzer import analyze_video_with_gemini
    ...
    # Fallback: text-only analysis from metadata
...
⚠️ *Video file could not be downloaded for visual analysis. Analysis is based on metadata only.*

---
**Instructions for downstream nodes:** Use this video as reference for style, tone, format and content structure. Analyze what makes this video engaging and replicate those elements."""


def process_brand_node(node: WorkflowNode, brand_context: str = "", config: Optional[NodeConfig] = None) -> str:
    """
    Process brand brief node - provides brand identity context.
    This is a SOURCE node that defines brand guidelines for content creation.
    """
    # Priority: node config > node brandData > request brand_context
    context = ""
...
**Example:**
"TechStartup Inc. - A B2B SaaS for project management. Target: Tech managers 25-45. Voice: Professional but approachable. Focus on productivity and team collaboration."
"""
...
---
**Instructions for downstream nodes:** All generated content MUST align with this brand identity. Maintain consistent voice, tone, and messaging throughout."""


def process_analyze_node(input_content: str, config: Optional[NodeConfig] = None) -> str:
    """
    Deep content analysis - breaks down WHY content works.
    Identifies viral mechanics, psychological triggers, and replicable patterns.
    """
    if not input_content or input_content.strip() == "":
        return "⚠️ No input content to analyze. Connect a Video Input or Brand Brief node."
...
{input_content}"""
        ...
        prompt = f"""You are an expert viral content strategist. Analyze this content deeply to understand what makes it work.
...
## 📊 CONTENT STRUCTURE
- Overall format and flow
- Pacing and rhythm
- Information density

## 🧠 PSYCHOLOGICAL TRIGGERS
- Primary emotions evoked (curiosity, FOMO, relatability, etc.)
...
- Aspirations tapped into

## ✅ REPLICABLE ELEMENTS
List 5 specific, actionable elements that can be replicated:
1.
2.
3.
4.
...
Be specific and actionable. Focus on elements that can be adapted for new content."""
...
def process_extract_node(input_content: str, config: Optional[NodeConfig] = None) -> str:
    """
    Extract key elements - pulls out reusable components.
    Creates a "toolkit" of elements for content creation.
    """
    if not input_content or input_content.strip() == "":
        return "⚠️ No input content to extract from. Connect upstream nodes first."
...
{input_content}"""
        ...
        prompt = f"""Extract all reusable elements from this content analysis. Create a toolkit for creating similar content.
...
List 3-5 hook templates based on this content:
1. "[Hook template 1]"
2. "[Hook template 2]"
3. "[Hook template 3]"
...
## 🔑 SUCCESS FORMULA
Summarize the winning formula in one sentence:

Make everything specific and immediately usable."""
...
def process_style_node(input_content: str, config: Optional[NodeConfig] = None) -> str:
    """
    Style matching - creates detailed style guide for replication.
    Ensures consistent aesthetic across content.
    """
    if not input_content or input_content.strip() == "":
        return "⚠️ No input content for style matching. Connect upstream nodes first."
...
{input_content}"""
        ...
        prompt = f"""Create a detailed, replicable style guide based on this content.
...
## 🎤 VOICE & TONE
- **Primary tone:** [e.g., "Energetic but knowledgeable"]
- **Language level:** [casual/conversational/professional]
- **Personality traits:** [list 3-5 adjectives]
- **What to AVOID:**

## 🎬 VIDEO FORMAT
- **Primary format:** [talking head / text-on-screen / montage / POV / etc.]
- **Aspect ratio:** [9:16 vertical / 1:1 square]
- **Optimal length:** [X-Y seconds]
- **Opening style:**
- **Closing style:**

## ✂️ EDITING STYLE
- **Cut pacing:** [fast cuts every X sec / smooth / jump cuts]
- **Transitions:** [none / swipe / zoom / etc.]
- **Text animations:** [pop / slide / typewriter / none]
- **Effects to use:**
- **Effects to AVOID:**
...
- **Background style:**
- **On-screen text style:** [font type, size, position]

## 🎵 AUDIO GUIDELINES
- **Music genre/mood:**
- **BPM range:** [if applicable]
- **Voiceover style:** [natural/energetic/ASMR/etc.]
- **Sound effects:** [yes/no, type]

## 📱 PLATFORM OPTIMIZATION
- **Best posting time:** [based on content type]
- **Caption style:**
- **Engagement strategy:**

This style guide should allow anyone to recreate content with the same feel."""
...
def process_generate_node(input_content: str, config: Optional[NodeConfig] = None) -> str:
    """
    AI Script Generation - creates complete, ready-to-film scripts.
    The creative engine of the workflow.
    """
    if not input_content or input_content.strip() == "":
        return "⚠️ No input context for script generation. Connect Analysis, Extract, or Style nodes."
...
{input_content}"""
        ...
        prompt = f"""You are an expert viral content creator. Write a complete, production-ready TikTok script.
...
---
Create a COMPLETE SCRIPT following this exact format:

# 📱 TIKTOK SCRIPT

## ⏱️ TIMING: [X seconds total]

---

## 🎣 HOOK (0-3 seconds)
**[SPOKEN/TEXT]:** "[Exact words to say or show]"
...
[Add more beats as needed for longer content]

---

## 📢 CTA (Final 3-5 seconds)
**[SPOKEN]:** "[Exact call to action]"
**[VISUAL]:** [Final frame direction]
**[TEXT OVERLAY]:** "[On-screen text]"

---

## 🎵 AUDIO NOTES
- **Music:** [Specific suggestion or mood]
- **Voiceover style:** [How to deliver]
...
Make the script feel natural, engaging, and impossible to scroll past."""
...
def process_refine_node(input_content: str, config: Optional[NodeConfig] = None) -> str:
    """
    Polish & Improve - enhances existing content for maximum impact.
    Focuses on optimization and viral mechanics.
    """
    if not input_content or input_content.strip() == "":
        return "⚠️ No content to refine. Connect a Generate node or provide existing script."
...
{input_content}"""
        ...
        prompt = f"""You are a viral content optimization expert. Refine this script for MAXIMUM impact and engagement.
...
---
Improve the content with these priorities:

## 🎯 REFINEMENT CHECKLIST

### 1. HOOK OPTIMIZATION
- Is the first line a pattern interrupt?
- Does it create immediate curiosity?
- Can it be even more punchy?

### 2. PACING & FLOW
- Remove ALL filler words
- Tighten every sentence
- Ensure each beat earns its place

### 3. EMOTIONAL AMPLIFICATION
- Increase relatability
- Add specificity (specific > vague)
- Strengthen the emotional payoff

### 4. VIRAL MECHANICS
- Add a "save this for later" moment
- Include comment bait (question/controversy)
- Create share trigger ("tag someone who...")
...
[Provide the complete refined version with all improvements applied]
...
The refined version should feel noticeably more engaging and scroll-stopping."""
...
def process_script_output_node(input_content: str, config: Optional[NodeConfig] = None) -> str:
    """
    Final Script Output - production-ready formatted script.
    Clean, professional format ready to film from.
    """
    if not input_content or input_content.strip() == "":
        return "⚠️ No script content to format. Connect a Generate or Refine node."
...
{input_content}"""
        ...
        if output_format == "plain":
            format_instruction = "Output as PLAIN TEXT only - no markdown, no formatting symbols."
        elif output_format == "json":
            format_instruction = "Output as valid JSON with keys: hook, body (array of beats), cta, audio_notes, hashtags."
        else:
            format_instruction = "Use clean Markdown formatting with headers and bullet points."

        prompt = f"""Clean up and format this script for FINAL PRODUCTION USE.
...
Create a clean, professional final script that a creator can film from immediately.

Include:
1. ⏱️ Total duration estimate
2. 🎣 HOOK - Exact words with timing
3. 📝 BODY - Each beat with script + visual direction
4. 📢 CTA - Clear closing
5. 🎵 Audio/music guidance
6. 📱 Caption and hashtags

Remove any analysis, explanations, or meta-commentary.
Keep ONLY the actionable script content.
Make it scannable and easy to follow while filming."""
...
def process_storyboard_node(input_content: str, config: Optional[NodeConfig] = None) -> str:
    """
    Visual Storyboard - scene-by-scene breakdown for filming.
    Creates a visual production guide.
    """
    if not input_content or input_content.strip() == "":
        return "⚠️ No script content for storyboard. Connect a Script Output or Generate node."
...
{input_content}"""
        ...
        prompt = f"""Create a detailed VISUAL STORYBOARD for this TikTok script.
...
- **Format:** [talking head / montage / POV / etc.]
...
**Action:** [What creator does]
**Mood:** [Energy level]
**→ Transition:** [Cut/swipe/zoom to next]

---

## SCENE 2 | 0:03 - 0:08 | BEAT 1
┌─────────────────────────────────┐
│                                 │
│  [VISUAL DESCRIPTION]           │
│  [Camera angle, framing]        │
...
**Audio:** [Music fade/VO]
**Action:** [Final action]
**End Frame:** [What stays on screen]

---

## 📋 SHOT LIST SUMMARY
| Scene | Duration | Shot Type | Key Element |
|-------|----------|-----------|-------------|
| 1     | 3s       | Close-up  | Hook        |
| 2     | 5s       | Medium    | Point 1     |
[etc.]
...
Make this detailed enough that any creator can follow it to recreate the content."""
...
class WorkflowExecuteRequestV2(BaseModel):
    ...
    connections: List[Connection]
    brand_context: Optional[str] = None
    workflow_id: Optional[int] = None  # Optional: link to saved workflow
    workflow_name: Optional[str] = None  # Name for history (uses workflow name if id provided)
...
async def execute_workflow(
    ...
    # If workflow_id provided, get name and calculate run number
    if request.workflow_id:
        ...
        if wf:
            ...
            # Count existing runs for this workflow
    ...
    try:
        ...
        # Pre-check: estimate total cost and verify sufficient credits
        ...
        for node_id in execution_order:
            ...
            try:
                ...
                # Deduct credits for this node (AI nodes only)
        ...
        # Also update the parent workflow if linked
        if request.workflow_id:
