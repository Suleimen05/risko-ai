...
def search_channel(
    ...
    logger.info(f"[Search] User {current_user.id} searching channel: @{clean_username}")
...
async def add_competitor(
    ...
    if existing:
        if existing.is_active:
            ...
            db.commit()
            db.refresh(existing)
            logger.info(f"[Reactivate] User {current_user.id} reactivated competitor @{clean_username}")
            return CompetitorResponse.model_validate(existing)

    # Deduct credits
    await CreditManager.check_and_deduct("competitor_add", current_user, db)

    logger.info(f"[Add] User {current_user.id} adding competitor: @{clean_username}")

    # Check if search_data was passed (optimization: skip Apify call)
    if data.search_data:
        logger.info(f"[Cache] Using cached search data for @{clean_username} (no Apify call)")
        ...
        logger.info(f"[OK] User {current_user.id} added competitor @{clean_username} (fast mode)")
        return CompetitorResponse.model_validate(competitor)

    # Fallback: Fetch profile data from Apify (slower)
    logger.info(f"[Fetch] Fetching @{clean_username} from Apify (no cached data)")
...
def update_competitor(
    ...
    logger.info(f"[Update] User {current_user.id} updated competitor @{clean_username}")
...
def delete_competitor(
    ...
    if hard_delete:
        ...
        competitor.updated_at = datetime.utcnow()
        logger.info(f"[Deactivate] User {current_user.id} deactivated competitor @{clean_username}")
