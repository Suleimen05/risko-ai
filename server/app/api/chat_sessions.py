...
Keep the response focused and actionable. Language: respond in the same language as the user's message."""
    ...
    try:
        ...
        if "credit balance is too low" in error_str or "insufficient_quota" in error_str:
            ...
            raise Exception(f"{model_name}: на аккаунте нет кредитов. Пополните баланс в личном кабинете провайдера.")
        elif "invalid x-api-key" in error_str or "invalid api key" in error_str or "authentication_error" in error_str:
            raise Exception(f"Неверный API-ключ для {model}. Проверьте ключ в настройках (.env).")
        elif "rate_limit" in error_str or "429" in error_str:
            raise Exception(f"Превышен лимит запросов для {model}. Попробуйте через минуту.")
        else:
            raise e
...
class ChatMessageCreate(BaseModel):
    ...
    mode: Optional[str] = None  # Override session mode if provided
    model: Optional[str] = None  # Override session model if provided (gemini, claude, gpt4)
