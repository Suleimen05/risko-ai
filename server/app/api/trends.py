...
def search_trends(
    ...
    # ==========================================================================
        ...
        limit = 20
        logger.info(f"[Profile] Parsing user profile '{search_targets[0]}'...")
    ...
    if req.is_deep and processed_trends:
        logger.info(f"[Clustering] Clustering {len(processed_trends)} videos...")
    ...
    if req.is_deep and processed_trends:
        ...
        if saved_urls:
            ...
            logger.info(f"[Scheduler] Rescan scheduled in {req.rescan_hours}h for user {current_user.id}")
...
def clear_user_trends(
    ...
    logger.info(f"[Clear] Cleared {deleted_count} trends for user {current_user.id}")
