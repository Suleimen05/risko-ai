...
# =============================================================================
# SAVE VIDEO (create trend + favorite in one step)
# =============================================================================

from pydantic import BaseModel as PydanticBaseModel

class SaveVideoRequest(PydanticBaseModel):
    """Accept raw video data from frontend and create trend + favorite"""
    platform_id: str
...
def save_video_as_favorite(
    data: SaveVideoRequest,
    current_user: User = Depends(check_rate_limit),
    db: Session = Depends(get_db)
):
    """
    Save any video as favorite — creates a Trend record if it doesn't exist,
    then adds to favorites. Works for both Light and Deep analyze results.
    """
    # Check if trend already exists for this user + platform_id
    ...
    if existing_trend:
        ...
        # Parse views/likes/comments/shares to integers
        def parse_stat(val: str) -> int:
            try:
                cleaned = val.replace(',', '').replace(' ', '')
                if 'K' in cleaned.upper():
                    return int(float(cleaned.upper().replace('K', '')) * 1000)
                if 'M' in cleaned.upper():
                    return int(float(cleaned.upper().replace('M', '')) * 1000000)
                return int(float(cleaned))
            except (ValueError, TypeError):
                return 0

        stats = {
            "playCount": parse_stat(data.views),
            "diggCount": parse_stat(data.likes),
            "commentCount": parse_stat(data.comments),
            "shareCount": parse_stat(data.shares),
        }
    ...
    logger.info(f"⭐ User {current_user.id} saved video {data.platform_id} (trend {trend.id})")
